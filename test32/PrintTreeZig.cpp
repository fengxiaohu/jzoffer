//
// Created by xiaohu feng on 2022/8/4.
//
/*******************************************************************
...
std::vector<std::vector<int>> Print(BinaryTreeNode* pRoot)
{
  ...
  while(!deque.empty())
  {
    ...
    for(int i=0;i<size;i++)
    {
      if((level&1)==1) {
        ...
        if(node->m_pLeft) deque.push_back(node->m_pLeft);
        if(node->m_pRight) deque.push_back(node->m_pRight);
      }
      else {
        node = deque.back();
        deque.pop_back();
        ...
        if (node->m_pRight) deque.push_front(node->m_pRight);
        if (node->m_pLeft) deque.push_front(node->m_pLeft);
...
void Test1()
{
  ...
  vector<vector<int>> res =  Print(pNode8);
  printf("\n");
